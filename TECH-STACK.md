...
| `/api/v1/games` | Kivo Games: `GET /invites` (pending invitations), `GET /mine` (waiting + in-flight sessions), `POST /invite {targetUserId,kind}` (`10/min`, resolves-or-creates the pair's DM, posts the chip, enforces the per-plan concurrent-session cap), `GET /:id` (passage revealed only while `active`), `POST /:id/join` (auto-starts a 1v1 once everyone invited is in), `POST /:id/decline`, `POST /:id/start` (host fallback), `POST /:id/progress` (`120/min`, monotonic 0–1, broadcast-only — never written to the chip), `POST /:id/finish` (idempotent, server-derived WPM + place), `POST /:id/cancel` (host only); lifecycle actions `30/min` |
