...
│                        #   CustomEmoji, PlusRequest, GameSession
├── modules/             # auth, users, friends, conversations, messages, spaces,
│                        #   notifications, push, attachments, search, link-preview, ai, calls, admin,
│                        #   status, emoji (custom emoji CRUD), plus (UPI claims), games (Kivo Games
│                        #   sessions + arena presence), crypto, billing, voice
...
| `/api/v1/ai` | AI assist: `proofread`/`rewrite`/`translate`/`replies`/`summarize` via Groq (`openai/gpt-oss-20b` fast, `openai/gpt-oss-120b` quality) → Gemini fallback, 20/min + daily quota, 1h cache (hits free) |
...
| `/api/v1/games` | Kivo Games: `GET /invites` (pending invitations), `GET /mine` (waiting + in-flight sessions), `POST /invite {targetUserId,kind}` (`10/min`, resolves-or-creates the pair's DM, posts the chip, enforces the per-plan concurrent-session cap), `POST /practice {difficulty}` (`10/min`, solo vs bot — born `active` with passage, no DM/chip), `GET /leaderboard` (weekly best-WPM board + my `User.arena` snapshot: xp/level/streaks/rank), `GET /series/:seriesId` (best-of-3 standings), `GET /:id` (passage revealed only while `active`), `POST /:id/join` (auto-starts a 1v1 once everyone invited is in), `POST /:id/decline`, `POST /:id/start` (host fallback), `POST /:id/progress` (`120/min`, monotonic 0–1, broadcast-only — never written to the chip), `POST /:id/finish` (idempotent, server-derived WPM + place, feeds `User.arena` progression), `POST /:id/cancel` (host only), `POST /:id/rematch` (next series game in the same DM), chess `GET /:id/moves?square=` (legal targets, read-only) + `POST /:id/move {from,to,promotion?}` (FEN-validated, lazy clock deduct, terminal detection, one DB write) + `POST /:id/resign` (either player); lifecycle actions `30/min` |
...
| `Message` | conversationId + senderId, content (4000), type `text/system/poll/game`, replyToMessageId (inline quote-replies), **threadId** (null = main timeline; set = reply inside the thread rooted at that message), reactions (subdocs), deliveredTo, readBy (`readBy` drives receipts — `message:read` carries `upToMessageId`/`upToCreatedAt`/`readAt` anchor), mentions, **attachments** (embedded; kind enum `image`/`document`/`audio`), **audioDuration** (seconds, voice messages only), **forwardedFromId + forwardedFromName** (forward attribution), **pinnedAt + pinnedBy** (pin is a timestamp, not a collection), **savedBy** (`[{userId, savedAt}]` per-user bookmarks), `isEdited` + `editHistory` (cap 10), `scheduledAt` + `status` (`sent`/`scheduled`), `forwardCount`, `poll` (when `type:poll`), **`game`** (Kivo Games card when `type:game` — `sessionId`, `kind`, `role` `invite`/`result`, `status`, embedded players snapshot, `winnerId`/`winnerName`, `startedAt`/`finishedAt`), `isDeleted` — **removed 2026-09-08:** `expireAt` + `status:"expired"` auto-delete |
...
| `GameSession` | one doc per Kivo Game (not embedded in the message): `kind` (`typing`), `status` (`pending`/`active`/`finished`/`cancelled`), `conversationId` (indexed), `messageId` (invite chip) + `resultMessageId` (result chip — guards against posting the result twice), `createdBy`, embedded `players` (`userId`, denormalized `displayName`, `status` `invited`/`joined`/`declined`/`left`, `progress` 0–1, `place`, `wpm`, `accuracy`, `elapsedMs`, `finishedAt`), server-picked `passage`, `startedAt`/`finishedAt`/`expiresAt`, `winnerId`; indexes: conversationId+status+createdAt, status+expiresAt (abandonment sweep), players.userId+status+updatedAt |
...
- Clients emit `typing:start/stop` (re-broadcast), `conversation:focus/blur` (DM notification suppression — checked in `createForMessage` before fan-out), `message:delivered` (receipt ack), call signaling `call:ring/accept/decline/end` (relayed with membership + block guards; 30s ring registry → `call:missed`), and `arena:enter`/`arena:leave` for the Kivo Games lobby.
- **Kivo Games** layers two more event groups on the same socket, with no new transport: an in-memory **arena roster** (`arena:enter`/`arena:leave` maintain a `Map<userId, {sockets, profile, blocked}>`; `arena:roster` is re-broadcast to every member on any change — safe to fan out because each member's profile and block list are cached at enter time, so a broadcast needs no DB reads and blocked pairs are filtered in both directions) and the conversation-scoped session lifecycle — `game:invited` (per-user nudge so an open arena surfaces the invite) plus `game:updated`/`game:started`/`game:progress`/`game:finished`/`game:cancelled` into the conversation room. `game:started` is the **only** event that carries the passage (`publicGame(session, { includePassage: true })`), and every emit that includes players uses the same full `publicPlayers()` shape so a client can replace its copy wholesale without silently dropping fields like `status`.
- **Arena presence is a separate map from presence** and carries the same single-instance constraint — the roster assumes one server process, like the rest of the realtime layer.
...
- **AI is on-device first, cloud as fallback**: Chrome Built-in AI (Translator/Proofreader/Rewriter/Summarizer/Prompt) handles grammar/translate/rewrite/summarize with zero server cost and no data leaving the device; `frontend/lib/ai.js` falls back to `POST /api/v1/ai/*` (same-origin proxy, so keys never touch the browser and no CSP change was needed) only when on-device is unavailable. The backend (`backend/src/lib/ai-providers.js`, no extra deps) tries Groq's free production models first and fails over to Gemini on 429/5xx/empty/retired-model-404 — the llama models moved to Enterprise in Sep 2026, so defaults are `openai/gpt-oss-20b`/`120b`. Daily quota (free 30 / Plus 100, `PLAN_LIMITS.aiDailyMax`) plus a 1h prompt cache keeps one free Groq key (~14.4k RPD) sufficient for hundreds of DAU; cache hits don't burn quota. Nothing AI-generated auto-sends — every surface previews (`ThinkingIndicator` + `StreamingText`) and inserts into the composer.
...
- **Kivo Games sessions get their own collection, not a message**: a `GameSession` holds live per-player progress, the server-picked passage, and the finish stats, while chat holds only a thin `game` card message pointing at it (`Message.type: "game"` + `Message.game`, `role: "invite"`/`"result"`) — the same split polls use, but with room for games that carry real rule state (chess, ludo). The card is kept current with `message:edited` so the chip flips waiting → live → finished with no refetch, and the **result is posted as a new message** attributed to the winner (guarded by `GameSession.resultMessageId`) so the conclusion earns its own notification and unread badge instead of being invisible history.
- **Kivo Games keeps its race rules in a dependency-free module**: `modules/games/games.rules.js` holds every pure decision (passage picking and the constants, `computeWpm`, `recordFinish`, `canRecordRunnerUpFinish`, series `seriesStandings`, practice bots `practiceBotFor`/`pickPracticeBotWpm`/`botProgressAt`, progression `arenaWeekKey`/`levelForXp`/`xpToNextLevel`/`utcDayKey`/`nextArenaStats`, chess `games.chess.js` wrapping `chess.js` (legal generation, mate/stalemate/draw detection, lazy-clock math)) with no Mongoose or socket import and **zero timers**, so the rules that decide **who won, who levels up and who tops the board** are unit-testable without a database — the same split as `src/lib/totp.js` + `totp.test.js`. `games.service.js` does the I/O and orchestration and imports from it. Chess sessions carry only FEN + SAN history + clock anchors (10 min each, 7-day abandonment); flags settle lazily on move/read, moves cost one DB write, and per-move chip writes are skipped (emit only) to stay light on free-tier hosting.
- **Kivo Games is server-authoritative end to end**: the server picks the passage and reveals it only once a race is `active`, owns the clock (WPM = characters ÷ 5 per minute, clamped to 400), assigns finish places, and treats `POST /:id/finish` as idempotent — a retry cannot claim two places, and a finish that lands after the opponent already won returns the authoritative result instead of a `GAME_NOT_ACTIVE` error. Clients report only `progress` (clamped 0–1 and forced monotonic so a reload cannot rewind) and "finished"; typing feels instant because the viewer's own progress bar renders locally from keystrokes while the server ping is throttled to every ~5% and at most once per 250 ms.
- **The countdown is a shared instant, not a client flourish**: activation stamps `startedAt` `COUNTDOWN_MS` (3 s) in the future instead of *now*, so both players count down to the same absolute moment and the race clock, the live WPM and the unlocked input all begin together — nobody's WPM is charged for the wait. That also means "is the countdown still running?" is derived from the timestamp rather than a local timer, so opening an already-running race shows none, a late joiner gets only the time that is left, and the client clamps the window to `RACE_COUNTDOWN_MS` so a badly skewed device clock cannot turn it into a minute-long wait. Finishing before `startedAt` is rejected (`GAME_NOT_STARTED`) and progress pings during the countdown are ignored.
- **A photo finish keeps both times, and only a photo finish**: `LATE_FINISH_WINDOW_MS` (15 s) lets a runner-up's already-in-flight finish be recorded as place 2 with a server-derived time — place is computed from how many have finished, so it can never steal the win — while anything later is discarded rather than inventing a time. It is announced as `game:updated`, **never** a second `game:finished`, because that event is what drives the winner/loser flash and a replay would fire on both screens. Where no second time exists (the usual case, since the race stops at the first finisher) the result reports the runner-up's progress at the line instead of fabricating a margin.
- **An abandoned race is reaped, an active one never is**: `RACE_DEADLINE_MS` is 10 minutes but every progress ping pushes `expiresAt` back, and pending invites expire after 30 minutes — so a slow typist is never cut off mid-race while a chip can still never sit in a stale "waiting" state. A 5-minute `startGameSweep()` (`abandonStaleGames()`, mirroring `closeExpiredPolls()`) cancels anything past its deadline and emits `game:cancelled`.
