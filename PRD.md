...
| Voice Messages | **Complete** | Hold-to-record mic (MediaRecorder), slide-up-to-cancel, **waveform** player — 32-bar deterministic pseudo-waveform (`waveformHeights` seeded by `fileId`/`url`, `useMemo`), `tabular-nums` timer (`m:ss / m:ss`), shared `<audio>` singleton (one `play` stops previous), `role="slider"` seekable waveform (`←/→ ±5s`, `Home`/`End`, `Space` toggle), responsive (`min-w 220→260 sm:max-w 340`, `w 2.5→3px`, `gap 2px`), `passive` `timeupdate`, no canvas/network waveform fetch; `Message.audioDuration` + `kind:audio` `url` preserved on forward |
...
- **3-2-1 countdown:** activation is not immediately playable. `startedAt` is stamped `COUNTDOWN_MS` (3000 ms) in the future so both players count down to the **same absolute moment** and the race clock, the live WPM and the input all begin together on "GO" — nobody's time is charged for the wait, and a late joiner sees exactly the time that is left (opening an already-running race shows no countdown at all). `POST /:id/finish` is rejected with `GAME_NOT_STARTED` before that instant, and progress pings during the countdown are ignored. The client mirrors the window as `RACE_COUNTDOWN_MS`.
...
- **Photo finish:** a runner-up's finish request is normally already in flight when the winner crosses the line, so a finish landing within `LATE_FINISH_WINDOW_MS` (15 s) of `finishedAt` is **recorded** (place 2, with the same server-derived elapsed/WPM) and announced as `game:updated` — never a second `game:finished`, which would replay the winner/loser flash on both screens. Place 2 is already decided by then, so recording it can never change who won; it just means a genuine photo finish keeps both times, and the result can report the real margin (`Won by 0.4s`). Without a second finisher the screen reports the runner-up's **progress at the line** (`reached 94%`) rather than inventing a gap.
- **Result card:** a **new** `type: "game"` message with `role: "result"` is posted, attributed to the winner and guarded by `GameSession.resultMessageId`, so the outcome earns its own notification and unread badge rather than being invisible history. The invite chip is kept current with `message:edited`, so it flips waiting → live → finished with no refetch. The result **chip** also leads with the margin when one exists (`won by 0.4s`).
...
- **Kivo Games depth:** Chess/Ludo tables, tournaments, spectators (shipped: rematch + best-of-3, win/loss record, runner-up finishes, practice bots, XP/levels/streaks/weekly board).
