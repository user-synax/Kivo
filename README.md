...
- 🎮 **Kivo Games** — a full-screen arena at **`/games`** with a 1v1 **Typing Race**, solo **Practice vs bots** (Easy/Medium/Hard), one-tap **Rematch best-of-3** in the same DM thread, and progression that sticks: **XP levels** (win +100, loss +25), **win streaks**, **daily play-streaks**, and a live **weekly WPM board** with a Monday reset. Fully server-authoritative (the server picks the passage, times the race, computes WPM and assigns places), chat carries only a one-line chip that deep-links back to the arena, and a green/red takeover flash + synthesized stinger announces the result.
...
- **Kivo Games (Typing Race)** — full-screen `/games` arena with live lobby presence (`arena:enter/leave` → `arena:roster`), 1v1 invites that resolve-or-create the pair's DM and post a chip there, a **server-authoritative** typing race (server-picked passage of 6, server-observed start time, WPM derived from the server clock and clamped to 400, server-assigned finish places), live progress over `game:progress`, and a **result chip posted as its own message** — plus a full-screen win/lose flash with a Web Audio stinger (`Settings → Sounds → Game Results`), solo **Practice vs bots** (`POST /practice`), one-tap **Rematch best-of-3** (`POST /:id/rematch`, `GET /series/:seriesId`), and progression (XP levels, win + daily streaks, weekly WPM board via `GET /leaderboard`, all stored on `User.arena`)
...
- **1v1 Chess** — pick **Chess** on the arena tabs and invite the same way: colors drawn randomly, tap-to-move with server-computed legal targets, promotion picker, move list, **10-minutes-each lazy clocks** (no server timers — derived on every move/read), checkmate/stalemate/repetition/material/fifty-move detection, resign (two taps), and best-of-3 rematches in the same thread. Wins feed XP/streaks; draws pay +25 each
- **3-2-1-GO countdown** — the server stamps the start three seconds ahead, so both players count down to the same instant (a blip per number, a brighter note on GO) and the clock, WPM and input all begin together — nobody's time is charged for the wait. The passage stays blurred until GO, so there is no head start either way
- **It gets tense at the end** — a bar past **80%** pulses amber, and the footer warns you when your opponent is nearly there
...
- **Photo finishes are called out** — a finish landing within 15s of the winner's is recorded as place 2 (never changing who won), so a genuine photo finish shows the real margin (**"won by 0.4s"**) over a **Photo finish** badge; otherwise the screen reports the honest alternative, how far the runner-up got (`reached 94%`)
- **Win/lose takeover** — a full-screen green **You Win** / red **You Lose** flash with a synthesized stinger (`Settings → Sounds → Game Results`)
- **Rematch best-of-3** — the result screen carries the series: one tap starts Game 2 (or the decider) in the same DM thread, first to 2 wins takes it, and an opponent's rematch arrives as an Accept banner
- **Practice vs bots** — Rookie (~30 WPM), Dash (~55), Blaze (~80) with instant start and no DM thread; XP only, so bots can't be farmed for rank
- **Levels, streaks, weekly board** — ranked win **+100 XP** / loss **+25** (practice +30/+10) on a root-curve level track (L1 0–99, L2 at 100, L3 at 400 — your first win always levels you up); **win streaks** reset on any ranked loss while **daily play-streaks** tick on UTC days with any finished race; the **weekly board** ranks best WPM and resets Monday 00:00 UTC
