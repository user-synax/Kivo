...
const revealJitter = (weekIndex, dayIndex) =>
  Math.round(((((weekIndex * 7 + dayIndex) * 137) % 97) / 97) * LEVEL_REVEAL_JITTER);
...
const fetchContributions = username => {
  ...
  const url = new URL(`/v4/${encodeURIComponent(username)}?y=last`, CONTRIBUTIONS_API);
...
const useGitHubContributions = username => {
  const [state, setState] = useState(IDLE_FETCH_STATE);
...
const createPlaceholderData = () => {
  ...
  return eachDayOfInterval({ start: subDays(today, 7 * 52), end: today }).map((day) => ({
...
const BlockTooltipApiContext = createContext(null);
const BlockTooltipStateContext = createContext(null);
...
const ContributionGraphBlockTooltip = () => {
  ...
  return createPortal(<div
    ...
    <div
      className="relative rounded-lg border border-white/10 bg-zinc-950 px-2.5 py-1 text-white shadow-[0_8px_26px_-8px_rgba(0,0,0,0.55)] dark:border-black/10 dark:bg-zinc-100 dark:text-zinc-950">
      {payload.content ?? (
        <div
          className="flex items-baseline gap-1.5 whitespace-nowrap text-[11px] leading-none">
...
export const ContributionGraph = ({
  ...
  const weeks = useMemo(() => groupByWeeks(resolvedData, weekStart), [resolvedData, weekStart]);
  ...
  const year =
    resolvedData.length > 0
      ? getYear(parseISO((resolvedData.at(-1)).date))
      : new Date().getFullYear();

  let totalCount = resolvedData.reduce((sum, activity) => sum + activity.count, 0);
  ...
  if (fetched.status === "error") {
    return (
      <div
        ...
        {...props}>Unable to load contributions for @{username}.
              </div>
    );
  }
...
export const ContributionGraphBlock = ({
  ...
  return (
    <rect
      ...
      className={cn("origin-center [transform-box:fill-box]", LEVEL_CLASSES, loading
        ? "animate-pulse"
        : "transition-transform duration-200 ease-out hover:scale-125", showEntrance &&
        (activity.level === 0
          ? "animate-[iconiq-cg-fade_0.4s_ease-out_backwards]"
          : "animate-[iconiq-cg-pop_0.45s_cubic-bezier(0.22,1,0.36,1)_backwards]"), "motion-reduce:animate-none", className)}
