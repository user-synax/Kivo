...
export function NotificationCenter({
  ...
  return (
    <AnimatePresence>
      {open && (
        <>
          {/* Backdrop — centers focus on mobile/desktop */}
          <motion.button
            type="button"
            ...
            {!loading && hasMore && nextCursor && (
              <div className="flex justify-center px-4 py-2">
                <button
                  type="button"
                  onClick={onLoadMore}
                  className="rounded-full border border-[var(--border)] px-3 py-1 text-[12px] font-medium text-[var(--text-muted)] hover:bg-[var(--hover)] hover:text-[var(--text-primary)]"
                >
                  Load more
                </button>
              </div>
