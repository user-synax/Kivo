...
export function GroupCreateModal({ open, onClose, onCreated }) {
  ...
  const body = (
    <>
      <div className="flex items-center gap-3.5">
        <Avatar name={name || "Group"} avatarStyle={null} url={avatarPreview} size="lg" />
        <div className="min-w-0 flex-1">
          <input value={name} maxLength={50} onChange={(e) => setName(e.target.value)} placeholder="Group name" aria-label="Group name" className={inputCls} />
          <input ref={fileRef} type="file" accept="image/png,image/jpeg,image/webp,image/gif" onChange={handleAvatar} className="hidden" />
          <button type="button" onClick={() => fileRef.current?.click()} className="mt-2 text-[12px] text-[var(--text-muted)] transition-colors hover:text-[var(--text-primary)]">
      ...
      <div className="mt-5">
        <label className="flex items-center gap-2 rounded-[var(--radius-inputs)] border border-[var(--border)] bg-[var(--bg-surface)] px-3 py-2 transition-colors duration-200 focus-within:border-[var(--accent)]">
          ...
          <input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Select friends…" aria-label="Search friends" className="w-full min-w-0 bg-transparent text-sm text-[var(--text-primary)] placeholder:text-[var(--text-muted)] focus:outline-none" />
        </label>
      </div>
      {error && <p className="mt-4 rounded-lg border border-[var(--border)] bg-[var(--bg-base)] px-3 py-2 text-[12px] text-[var(--text-muted)]">{error}</p>}
      <div className="mt-4 flex flex-col gap-2">
        {filtered.length === 0 && <p className="px-1 py-6 text-center text-[13px] text-[var(--text-muted)]">No friends match &ldquo;{query.trim()}&rdquo;.</p>}
        {filtered.map((f) => {
  ...
  const footer = (
    <div className="flex items-center justify-between gap-3 border-t border-[var(--border)] px-5 py-4 pb-[max(env(safe-area-inset-bottom),1rem)] md:pb-4">
      <p className="text-[12px] text-[var(--text-muted)]">{selected.size} selected</p>
      <button type="button" onClick={submit} disabled={busy || selected.size < 2 || !name.trim()} className={btnPrimary}>
        <Users className="h-3.5 w-3.5" strokeWidth={2} /> Create group
      </button>
    </div>
  );
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-end justify-center">
        ...
        <motion.div
          ...
          className="relative z-10 flex max-h-[85dvh] w-full flex-col overflow-hidden rounded-t-3xl border border-[var(--border)] bg-[var(--bg-elevated)] shadow-[0_24px_60px_-20px_rgba(0,0,0,0.45)]"
        >
          <div className="mx-auto mt-3 h-1.5 w-9 shrink-0 rounded-full bg-[var(--border)]" aria-hidden="true" />
          <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-3.5">
            <h2 className="text-base font-semibold tracking-tight text-[var(--text-primary)]">New group</h2>
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8">
      ...
      <div role="dialog" aria-modal="true" aria-label="Create group" className={`t-modal relative z-10 flex max-h-[85vh] w-full max-w-md flex-col overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--bg-elevated)] shadow-[0_24px_60px_-20px_rgba(0,0,0,0.45)] ${show ? "is-open" : "is-closing"}`}>
        <div className="flex items-center justify-between border-b border-[var(--border)] px-5 py-3.5">
          <h2 className="text-base font-semibold tracking-tight text-[var(--text-primary)]">New group</h2>
          <button type="button" onClick={onClose} aria-label="Close" className="flex size-9 items-center justify-center rounded-full border border-[var(--border)] text-[var(--text-muted)] transition-colors duration-150 hover:bg-[var(--hover)] hover:text-[var(--text-primary)]">
            <X className="w-5 h-5" />
          </button>
        </div>
        <div className="flex-1 overflow-y-auto px-5 py-5">{body}</div>
