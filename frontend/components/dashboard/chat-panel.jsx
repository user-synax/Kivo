...
import { EditHistoryModal } from "@/components/chat/edit-history-modal";
import { ScheduledPicker } from "@/components/chat/scheduled-picker";
            ...
            <div className={`flex min-w-0 flex-col max-w-full w-fit ${mine ? "ml-auto items-end" : "mr-auto items-start"}`}>
              ...
              >
                <MessageBubble
                  ...
                  onCopy={hasCopy ? () => a.handleCopyMessage(m) : undefined}
                  onSaveToggle={
                  ...
                  onPinToggle={
                    realMessage ? () => a.handlePinToggle(m) : undefined
                  }
                  onSelectMode={
                    realMessage ? () => a.enterSelectMode(m) : undefined
                  }
                  onShare={
                    !m.isDeleted ? () => a.handleShareMessage(m) : undefined
                  }
                  onThread={
                    realMessage &&
                    !m.isDeleted &&
                  ...
                  blockBusy={blockBusy}
                  blockName={isDm && otherId ? a.otherName : ""}
          ...
          {!m.isDeleted && threadSummaries[m.id]?.replyCount > 0 && (
            ...
            >
              ...
              <span>
                ...
                {threadSummaries[m.id].replyCount === 1 ? "reply" : "replies"}
              </span>
...
export function ChatPanel({
  ...
  const inThisCall = Boolean(
    callSession && callSession.conversationId === convId,
  );
  ...
  const handlePollVote = async (message, optionIds) => {
    ...
    try {
      const updated = await apiPost(
        `/api/v1/messages/${message.id}/poll/vote`,
        { optionIds },
      );
      setMessages((prev) =>
  ...
  const handlePollEnd = async (message) => {
    ...
    try {
      const updated = await apiPost(
        `/api/v1/messages/${message.id}/poll/end`,
        {},
      );
      setMessages((prev) =>
        prev.map((m) => (m.id === updated.id ? updated : m)),
  ...
  const openHistory = async (messageId) => {
    try {
      const res = await apiGet(`/api/v1/messages/${messageId}/history`);
  ...
  const openSchedulePicker = () => setShowSchedulePicker(true);
  const handleSchedule = async (scheduledAt) => {
    if (!convId || !text.trim()) return;
    try {
      const res = await apiPost(`/api/v1/conversations/${convId}/messages`, {
        content: text.trim(),
        scheduledAt: new Date(scheduledAt).toISOString(),
      });
      setText("");
      clearDraft(draftsKey(userId), convId);
      setScheduledList((prev) => [...prev, res]);
      setShowSchedulePicker(false);
      showNotice("Message scheduled");
    } catch (e) {
      setSendError(e.message || "Could not schedule");
      showNotice(e.message || "Could not schedule");
    }
  };
  const loadScheduled = async () => {
    ...
    try {
      let res;
      try {
        res = await apiGet(`/api/v1/conversations/${convId}/scheduled`);
      } catch {
        res = await apiGet(`/api/v1/messages/${convId}/scheduled`);
      }
  ...
  useEffect(() => {
    if (convId) loadScheduled();
  }, [convId]);
  const cancelScheduled = async (messageId) => {
    try {
      await apiDelete(`/api/v1/messages/scheduled/${messageId}`);
      setScheduledList((prev) => prev.filter((m) => m.id !== messageId));
      showNotice("Scheduled message cancelled");
    } catch (e) {
      showNotice(e.message || "Could not cancel");
    }
  };
  ...
  useEffect(() => {
    ...
    const onNew = (msg) => {
      ...
      reconcile(msg);
      // If this was a scheduled message now delivered, remove it from scheduled list
      if (msg?.id) {
        setScheduledList((prev) => prev.filter((m) => m.id !== msg.id));
    ...
    const onRead = (payload) => {
      ...
      setMessages((prev) =>
        prev.map((m) => {
          ...
          if (m.readBy?.some((r) => String(r.userId || r) === readerId))
            return m;
  ...
  const endRecordingPress = () => {
    if (recorderRef.current) {
      ...
      if (held < 250) {
        ...
        setRecording((prev) =>
          prev ? { ...prev, handsfree: true, cancelling: false } : prev,
        );
        return;
      }
      finishRecording();
    ...
    const t = setInterval(() => {
      setRecording((prev) =>
        prev
          ? {
              ...prev,
              seconds: Math.floor((Date.now() - prev.startedAt) / 1000),
            }
          : prev,
      );
    }, 500);
                ...
                {scheduledList.length > 0 && (
                  <div className="mb-2 flex flex-col gap-1.5">
                    <button
                      ...
                      className="self-start rounded-full border border-[var(--border)] bg-[var(--bg-surface)] px-3 py-1 text-xs font-medium text-[var(--text-muted)] transition-colors hover:bg-[var(--hover)] hover:text-[var(--text-primary)]"
                    >
                    ...
                    {showScheduledList && (
                      <div className="flex max-h-40 flex-col gap-1 overflow-y-auto rounded-lg border border-[var(--border)] bg-[var(--bg-surface)] p-2">
                        {scheduledList.map((m) => (
                          <div
                            key={m.id}
                            className="flex items-center gap-2 rounded-md px-2 py-1.5 hover:bg-[var(--hover)]"
                          >
                            ...
                            <button
                              ...
                              className="flex size-6 shrink-0 items-center justify-center rounded-full text-[var(--text-muted)] hover:bg-[var(--hover)] hover:text-[var(--destructive)]"
                  ...
                  {recording ? (
                    <div className="flex w-full items-center gap-3 px-1 py-1.5">
                      ...
                      <span className="shrink-0 text-sm tabular-nums text-[var(--text-primary)]">
                        {formatRecordTime(recording.seconds)}
                      </span>
                      {!recording.cancelling && !recording.handsfree && (
                        <ArrowUp
      ...
      <AnimatePresence>
        {composerMenuOpen && (
          <div className="fixed inset-0 z-[65] flex items-end justify-center sm:items-center">
            ...
            >
              <div
                className="hidden h-1.5 w-9 self-center rounded-full bg-[var(--border)] max-sm:mt-2 max-sm:block"
                aria-hidden
              />
              ...
              <p className="px-4 pb-3 text-center text-[11px] text-[var(--text-muted)]">
                Polls work in DMs, groups & Space channels
              </p>
            </motion.div>
          </div>
        )}
            ...
            <div className="min-h-0 flex-1 overflow-y-auto">
              ...
              {!forwardError && forwardConvs.length === 0 ? (
                ...
                <ul className="flex flex-col gap-0.5 p-2">
                  {forwardConvs.map((c) => {
                    const otherP =
                      c.type === "dm" ? otherParticipant(c, userId) : null;
                    const label =
                      c.type === "dm"
                        ? participantName(otherP)
                        : c.name || c.type;
                    const fIsPlus =
                      c.type === "dm" ? Boolean(otherP?.isPlus) : false;
                    const fAvatarStyle =
                      c.type === "dm" ? otherP?.avatarStyle || null : null;
                    const fAvatarUrl =
                      c.type === "dm"
                        ? otherP?.avatarUrl || c.avatarUrl
                        : c.avatarUrl;
                    return (
                      <li key={c.id}>
                        <button
