    ...
    <ContextMenu>
      <ContextMenuTrigger>
        <button
          ...
          className={`group flex w-full items-center gap-3 rounded-xl px-2.5 py-2.5 text-left select-none touch-manipulation [-webkit-touch-callout:none] [-webkit-tap-highlight-color:transparent] will-change-transform transition-[transform,background-color] duration-150 ease-[${EASE}] motion-reduce:animate-none animate-[t-item-in_0.4s_${EASE}_both] hover:cursor-pointer active:bg-[var(--hover)] active:scale-[0.98] aria-expanded:bg-[var(--accent-soft)] aria-expanded:scale-[0.992] ${pressBg} ${pressScale} ${
...
function ConversationSection({ label, items, selectedId, onSelect, onMarkUnread, onRemove, onPin, onMute, onViewInfo, onBlock, baseIndex = 0, collapsed, onNewGroup }) {
