...
export function UserPanel({ profile, loading, online, lastActiveAt, conversationCreatedAt, conversation, onConversationUpdate }) {
  ...
  const otherId = profile?.id || conversation?.otherParticipantIds?.[0] || null;
  const otherName = profile?.displayName || profile?.username || "this user";

  const handleBlock = async () => {
    if (!otherId || blockBusy) return;
    if (!window.confirm(`Block ${otherName}? You won't receive messages from them and the friendship will be removed.`)) return;
    setBlockBusy(true);
    try {
  ...
  const handleUnfriend = async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent("kivo:friend-removed", { detail: { friendId: otherId } }));
    } catch (err) {
