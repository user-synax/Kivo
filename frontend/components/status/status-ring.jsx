...
export function StatusRing({ name, url, avatarStyle, isPlus, hasUnseen, isMine, size="md", shape="rounded" }) {
  ...
  return (
    <div className={`relative shrink-0 rounded-full ${ringClass} ${isMine ? "p-0.5" : "p-0.5"}`}>
      <Avatar name={name} url={url} avatarStyle={avatarStyle} isPlus={isPlus} size={size} shape={shape} />
      {isMine && (
